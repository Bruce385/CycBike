...
import cyc3253.xmlg.cycbike.activity.BDInnerNaviActivity;
import cyc3253.xmlg.cycbike.dialog.CommomDialog;
import cyc3253.xmlg.cycbike.dialog.NaviSelectDialog;
import cyc3253.xmlg.cycbike.tools.LocationManager;

import static cyc3253.xmlg.cycbike.application.DemoApplication.APP_FOLDER_NAME;
import static cyc3253.xmlg.cycbike.application.DemoApplication.mSDCardPath;

/**
 * Created by GaoLei on 17/3/31.
 * 这个工具类实现了调用内置导航和打开第三方App导航
 * 1.assets中的文件必须拷贝到项目
 * 2.想使用内置导航，必须初始化导航， NavigatorUtil.initNavi(this);
 */
public class NavigatorUtil {
     ...
     * 弹出导航选择dialog
     */
    public static void showChoiceNaviWayDialog(final Activity activity, final LatLng startLL, final LatLng endLL,
                                               final String start_place, final String destination) {
...
//                    startNative_Baidu(activity, startLL, endLL, start_place, destination);
    ...
    private static void launchNavigatorViaPoints(final Activity activity, LatLng startLL, LatLng endLL) {
        ...
        final BNRoutePlanNode sNode = new BNRoutePlanNode(startLL.longitude, startLL.latitude, null,
                "从这里开始", BNRoutePlanNode.CoordinateType.BD09LL);
        final BNRoutePlanNode eNode = new BNRoutePlanNode(endLL.longitude, endLL.latitude, null,
                "到这里结束", BNRoutePlanNode.CoordinateType.BD09LL);
        if (sNode != null && eNode != null) {
            ...
            BaiduNaviManager.getInstance().launchNavigator(activity, points, 1, true,
			 ...
			 */
                ...
                public void onRoutePlanFailed() {
                    ...
                    Toast.makeText(activity, "计算路径失败", Toast.LENGTH_SHORT).show();
                }
            });
        }
    }
    ...
    public static void startGaodeNavi(Activity activity, LatLng pt1, LatLng pt2, String start_place) {
        try {
            ...
            intent.setData(android.net.Uri.parse("androidamap://navi?sourceApplication=yongche&poiname="
                    + start_place + "&lat=" + eLat + "&lon=" + eLon + "&dev=0&style=2"));
    ...
    public void showDialog(final Activity activity) {
        new CommomDialog(activity, "您尚未安装百度地图app或app版本过低，点击确认安装？",
                new CommomDialog.OnCloseListener() {
                    @Override
                    public void onClose(boolean confirm) {
                        if (confirm) OpenClientUtil.getLatestBaiduMapApp(activity);
                    }
                }).show();
    }
