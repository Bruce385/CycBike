...
import cyc3253.xmlg.cycbike.adapter.MyRouteAdapter;
import cyc3253.xmlg.cycbike.adapter.MyRouteDividerDecoration;
import cyc3253.xmlg.cycbike.bean.RouteRecord;
import cyc3253.xmlg.cycbike.database.RouteDBHelper;
...
//        bundle.putParcelable("routeContent",routeRecord );
    ...
    public List<RouteRecord> getAllPoints() {
        ...
        while (cursor.moveToNext()) {
            ...
            point.setCycle_date(cursor.getString(cursor.getColumnIndex("cycle_date")));
            point.setCycle_time(cursor.getString(cursor.getColumnIndex("cycle_time")));
            point.setCycle_distance(cursor.getString(cursor.getColumnIndex("cycle_distance")));
            point.setCycle_price(cursor.getString(cursor.getColumnIndex("cycle_price")));
            point.setCycle_points(cursor.getString(cursor.getColumnIndex("cycle_points")));
            routeList.add(point);
        }
        return routeList;
    }
    ...
    public List<RouteRecord> loadPage() {
        ...
        Log.d("MyRouteActivity", "PageId--------------" + PageId);
        Log.d("MyRouteActivity", "cursor.getCount()--------------" + cursor.getCount());
        Log.d("MyRouteActivity", "routeList.size()--------------" + routeList.size());
        Log.d("MyRouteActivity", "itemCount--------------" + itemCount);
