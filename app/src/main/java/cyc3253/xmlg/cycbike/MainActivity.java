...
import cyc3253.xmlg.cycbike.activity.BaseActivity;
import cyc3253.xmlg.cycbike.activity.CodeUnlockActivity;
import cyc3253.xmlg.cycbike.activity.MyRouteActivity;
import cyc3253.xmlg.cycbike.activity.NavigationActivity;
import cyc3253.xmlg.cycbike.activity.PersonalActivity;
import cyc3253.xmlg.cycbike.activity.RouteDetailActivity;
import cyc3253.xmlg.cycbike.activity.SettingsActivity;
import cyc3253.xmlg.cycbike.activity.WalletActivity;
import cyc3253.xmlg.cycbike.bean.BikeInfo;
import cyc3253.xmlg.cycbike.bean.User;
import cyc3253.xmlg.cycbike.callback.AllInterface;
import cyc3253.xmlg.cycbike.dialog.CommomDialog;
import cyc3253.xmlg.cycbike.custom.LeftDrawerLayout;
import cyc3253.xmlg.cycbike.fragment.LeftMenuFragment;
import cyc3253.xmlg.cycbike.map.MyOrientationListener;
import cyc3253.xmlg.cycbike.map.RouteLineAdapter;
import cyc3253.xmlg.cycbike.service.RouteService;
import cyc3253.xmlg.cycbike.tools.LocationManager;
import cyc3253.xmlg.cycbike.utils.Utils;
import overlayutil.OverlayManager;
import overlayutil.WalkingRouteOverlay;

import static cyc3253.xmlg.cycbike.bean.BikeInfo.infos;
...
//                    mBaiduMap.setOnMarkerClickListener(overlay);
    ...
    private void addOverLayout(double _latitude, double _longitude) {
        ...
        LatLng point = new LatLng(_latitude, _longitude); // 定义Maker坐标点
        // 构建MarkerOption，用于在地图上添加Marker
        MarkerOptions options = new MarkerOptions().position(point).icon(dragLocationIcon);
        mBaiduMap.addOverlay(options); // 在地图上添加Marker，并显示
        infos.clear();
        ...
        BikeInfo bikeInfo = new BikeInfo(_latitude - 0.0005,
                _longitude - 0.0005, R.mipmap.bike_xiaolan, "005", "50米", "0.5分钟");
        infos.add(bikeInfo);
        ...
        mBaiduMap.setOnMarkerClickListener(new BaiduMap.OnMarkerClickListener() {
            ...
            public boolean onMarkerClick(final Marker marker) {
                ...
                if (marker != null && marker.getExtraInfo() != null) {
                    ...
                    if (bikeInfo != null) updateBikeInfo(bikeInfo);
                }
                return true;
            }
        });
    ...
    private void initNearestBike(final BikeInfo bikeInfo, LatLng ll) {
        ...
        InfoWindow mInfoWindow = new InfoWindow(
                BitmapDescriptorFactory.fromView(nearestIcon), ll, -108, listener);
...
//            return null;
    ...
    protected void onStart() {
        ...
        mMenuFragment.setUserNum(user.getUsername());
        if (user.getNickName() != null) mMenuFragment.setUserName(user.getNickName());
        if (user.getHeadImg() != null)
            Glide.with(this).load(user.getHeadImg().getFileUrl()).into(mMenuFragment.getUserPhoto());
    }
    ...
    protected void onRestart() {
        ...
        if (RouteDetailActivity.completeRoute) backFromRouteDetail();
    }
    ...
    private void beginService() {
        ...
        mBaiduMap.clear();
        if (isServiceLive) mLocationClient.requestLocation();
    ...
    protected void onResume() {
        ...
        filter.addAction("Ending.MainActivity");
    ...
    public boolean onKeyDown(int keyCode, KeyEvent event) {
        if (keyCode == KeyEvent.KEYCODE_BACK && event.getAction() == KeyEvent.ACTION_DOWN) {
            if (bike_layout.getVisibility() == View.VISIBLE) {
                if (!Utils.isServiceWork(this, "com.biubike.service.RouteService")) cancelBook();
                return true;
            }
...
//                System.exit(0);
    ...
    private BroadcastReceiver broadcastReceiver = new BroadcastReceiver() {
        @Override
        public void onReceive(Context context, Intent intent) {
            unregisterReceiver(this); // 这句话必须要写，不写虽然能关闭，会报一堆错
            ((Activity) context).finish();
        }
    };

    protected void toastDialog() {
        new CommomDialog(MainActivity.this, "您确认要结束进程吗？", new CommomDialog.OnCloseListener() {
            @Override
